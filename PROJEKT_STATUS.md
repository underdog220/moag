...
## Aktueller Stand
**Phase Y komplett + Bug-Fix Listing:** Upload-Hub als dritte Top-Achse `/upload` mit DB-Persistenz live auf VDR:17900. Backend (Foundation + 10 Handler), Frontend-Skelett, alles auf VDR mit Volume `/home/underdog/moag-data → /data/moag`. PostgreSQL aktiv (Oberon-DB-Broker, gecached). `GET /api/v1/uploads` liefert korrekte Upload-Liste. 24 Upload-DB-Tests grün.
...
## Offene Punkte
- ~~Upload-Hub Listing-Endpoint crash~~ — behoben (2026-05-17, psycopg dict_row + COUNT AS n)
...
## Letzte Änderung
2026-05-17 — **Bug-Fix GET /api/v1/uploads HTTP 500** (psycopg dict_row + COUNT AS n). Live auf VDR grün.
