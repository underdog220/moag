...
- **Hardware-Priorisierung (2026-06-02):** `hardware_direct` (HwDirectPullPoller) wird bevorzugt gelesen; Fallback auf `hardware` (Heartbeat, bekannter null-Bug). Quell-Tooltip (ADR-004): "Direkt-Pull (vor Xs)" vs "Heartbeat (Lasten ggf. null)". Fallback-Logik einmalig im Adapter (`hub_client._map_nodes`), nicht im Frontend.
...
- **B3 — Diff-Highlight (Default an, Toggle):** zeilenbasiertes LCS (`_diff.ts`, MAX 4000 Zeilen) markiert geänderte Zeilen **zweifarbig**: Original-Seite rot (enthielt PII), anonymisierte Seite grün (Ersetzung), mit Legende. **Hinweis:** `oberon_pii.json` liefert nur PII-Typen/Längen, KEINE Offsets — echtes Offset-Highlighting ist daher nicht möglich, der Diff ist die buildbare Alternative.
- **Sync-Scroll:** alle Text-Spalten scrollen proportional synchron (`useSyncScroll`, ratio-basiert, Rückkopplungs-Guard) — direkter Zeilenvergleich beim Scrollen.
